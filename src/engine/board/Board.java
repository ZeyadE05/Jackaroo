...
public class Board implements BoardManager {
	...
	private ArrayList<Cell> validateSteps(Marble marble, int steps)
		...
		if (positionOnTrack == -1 && positionInSafeZone == -1)
			throw new IllegalMovementException(
					"Cannot move a marble that is not on track nor Safe Zone");

		if (positionOnTrack != -1) {
			int distanceToEntry = entryPosition - positionOnTrack;

			if (marble.getColour() == ownerColour && steps > 0
					&& distanceToEntry >= 0 && steps > distanceToEntry + 4)
				throw new IllegalMovementException("Rank is too high!");

			if (marble.getColour() == ownerColour && steps > 0
					&& distanceToEntry >= 0 && steps > distanceToEntry) {
				fullPath.addAll(track.subList(positionOnTrack,
						entryPosition + 1));
		...
		if (positionInSafeZone != -1) {
			if (steps < 0)
				throw new IllegalMovementException(
						"Cannot move a marble backwards in Safe Zone");
			...
			fullPath.addAll(safeZone.subList(positionInSafeZone,
					positionInSafeZone + steps + 1));
		}
	...
	private void validatePath(Marble marble, ArrayList<Cell> fullPath,
		...
		for (int i = 1; i < fullPath.size(); i++) {
			...
			if (cell.getMarble() != null) {
				if (i != fullPath.size() - 1) // counting marbles in my path
												// excluding target
					marbleCount++;

				if (cell.getCellType() == CellType.SAFE)
					throw new IllegalMovementException(
							"Cannot bypass my Safe Zone marbles!");

				if (cell.getCellType() == CellType.BASE
						&& track.indexOf(cell) == getBasePosition(cell
								.getMarble().getColour()))
					throw new IllegalMovementException(
							"Cannot bypass or land on marbles in their Base Cell!");
				...
				if (!destroy) {
					...
					if (ownerColour == cell.getMarble().getColour())
						throw new IllegalMovementException(
								"Cannot bypass or land on my own marble!");
					...
					if (cell.getCellType() == CellType.ENTRY
						...
						throw new IllegalMovementException(
								"Cannot bypass a marble blocking my Safe Zone!");

					if (marbleCount > 1)
						throw new IllegalMovementException(
								"Cannot bypass more than 1 marble in my path!");
				}
	...
	private void validateSwap(Marble marble_1, Marble marble_2)
		...
		if (trackPosition_1 == -1 || trackPosition_2 == -1)
			throw new IllegalSwapException(
					"Cannot swap marbles that are not on track.");

		Cell cell_1 = track.get(trackPosition_1);
		Cell cell_2 = track.get(trackPosition_2);

		if (ownerColour != marble_1.getColour()
				&& cell_1.getCellType() == CellType.BASE
				&& trackPosition_1 == getBasePosition(cell_1.getMarble()
						.getColour())
				|| ownerColour != marble_2.getColour()
				&& cell_2.getCellType() == CellType.BASE
				&& trackPosition_2 == getBasePosition(cell_2.getMarble()
						.getColour()))
			throw new IllegalSwapException(
					"Marbles that are in their Base Cell can not be selected as a swap target.");
	}
	...
	private void validateDestroy(int positionInPath)
		...
		if (positionInPath == -1)
			throw new IllegalDestroyException(
					"Cannot burn marbles that aren't on track.");

		Cell current = track.get(positionInPath);

		if (current.getCellType() == CellType.BASE
				&& current.getMarble() != null
				&& positionInPath == getBasePosition(current.getMarble()
						.getColour()))
			throw new IllegalDestroyException(
					"Cannot burn marbles that are safe in their Base Cell.");
	...
	private void validateFielding(Cell occupiedBaseCell)
		...
		if (occupiedBaseCell.getMarble().getColour() == gameManager
			...
			throw new CannotFieldException(
					"One of your marbles is already on your Base Cell");
	}
	...
	private void validateSaving(int positionInSafeZone, int positionOnTrack)
		...
		if (positionInSafeZone != -1)
			throw new InvalidMarbleException(
					"Cannot save marbles that are already in the Safe Zone.");

		if (positionOnTrack == -1)
			throw new InvalidMarbleException(
					"Cannot save marbles that aren't on track.");
	}
