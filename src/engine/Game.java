...
public class Game implements GameManager {
	...
	public Game(String playerName) throws IOException {
		...
		for (int i = 1; i < 4; i++)
			this.players
					.add(new CPU("CPU " + i, colourOrder.get(i), this.board));
	...
	public void editSplitDistance(int splitDistance)
			throws SplitOutOfRangeException {
	...
	public void fieldMarble() throws CannotFieldException,
		...
		if (marble == null)
			throw new CannotFieldException(
					"No marbles left in the Home Zone to field.");
			...
			if (player.getColour() == colour) {
				...
				if (handSize == 0)
					throw new CannotDiscardException(
							"Player has no cards to discard.");
