...
public class CPU extends Player {
	...
	public void play() throws GameException {
		...
		ArrayList<Marble> actionableMarbles = boardManager
				.getActionableMarbles();

		// Retrieve the current hand of cards and shuffle them to ensure
		// randomness.
		...
		for (Card card : cards) {
			...
			// Prepare a list to keep track of valid marble counts for the
			// action.
			ArrayList<Integer> counts = new ArrayList<>();
			for (int i = 0; i < 3; i++) { // Check for 0 or 1 or 2 marbles to
											// act upon.
				if (actionableMarbles.size() >= i) {
					...
					// Validate the size of the marble group against the card's
					// rules.
			...
			for (int i = 0; i < counts.size(); i++) {
				if (counts.get(i) == 0) {
					try {
						...
						// Ignore exceptions and continue trying other
						// possibilities.
					}
				} else if (counts.get(i) == 1) {
					// Attempt to act with one marble.
					ArrayList<Marble> toSend = new ArrayList<>();
					Collections.shuffle(actionableMarbles); // Shuffle marbles
															// for random
															// selection.
