...
public class Main extends Application {
	...
	public void handlePlay() {
		if (selectedCardView != null) {
			...
			if (!model.getFirePit().isEmpty()) {
				Card latest = model.getFirePit().get(
						model.getFirePit().size() - 1);
				if (firepit) {
	...
	public void updateTrackView() {
		for (int i = 0; i < 100; i++) {
			...
			if (cell.getMarble() != null) {
				...
				MarbleView c = new MarbleView(8, cell.getMarble(),
						getColor(cell.getMarble().getColour()));
		...
		for (int i = 0; i < safeZones.size(); i++) {
			...
			for (int j = 0; j < s.getCells().size(); j++) {
				...
				if (marble != null) {
					...
					int row = trackView
							.getRowIndex(safeZonesView.get(i).get(j));
					int col = trackView.getColumnIndex(safeZonesView.get(i)
							.get(j));
					trackView.getChildren().remove(safeZonesView.get(i).get(j));
					MarbleView c = new MarbleView(8, marble,
							getColor(marble.getColour()));
					c.setOnMouseClicked(e -> selectMarbleHandle(c));
	...
	public void updateHomeZoneView() {
		...
		for (int i = 0; i < players.size(); i++) {
			...
			homeZonesView.add(new HomeZoneView(getColor(players.get(i)
					.getColour()), homeZone));
		}
		renderHomeZones();
	...
	public void selectCardHandle(CardView card) {
		try {
			if (selectedCardView == null) {
				...
				System.out
						.println("Card selected: " + card.getCard().getName());
			...
			Card selectedCard = this.model.getPlayers().get(0)
					.getSelectedCard();
			if (selectedCard != null) {
				System.out.println("Currently selected card: "
	...
	public void selectSplitDistance() {
		...
		increase.setOnAction(event -> {
			if (splitDistance < 6) {
				splitDistance++;
				splitDistanceText.setText("Current Split Distance Is: "
						+ splitDistance);
			}
		});
	...
	public void selectMarbleHandle(MarbleView marble) {
		...
		if (selectedMarblesView.contains(marble)) {
			...
			if (selectedCardView != null) {
				try {
					this.model.getPlayers().get(0)
							.selectCard(selectedCardView.getCard());
				} catch (InvalidCardException e) {
					// Handle exception if needed
					System.err.println("Failed to reselect card: "
							+ e.getMessage());
				}
			}
			...
			for (MarbleView m : selectedMarblesView) {
				try {
					...
					// Handle exception if needed
					System.err.println("Failed to reselect marble: "
							+ e.getMessage());
				}
			}
	...
	public void renderSafeZones() {
		...
		addLineOfCircles(trackView, 23, 24, 4, false, true,
				safeZonesView.get(1));
	}
	...
	public void renderHomeZones() {
		...
		for (HomeZoneView h : homeZonesView) {
			...
			for (MarbleView marbleView : h.marbles) {
				marbleView
						.setOnMouseClicked(e -> selectMarbleHandle(marbleView));
			}
		}
	...
	public void addLineOfCircles(GridPane grid, int startRow, int startCol,
			int length, boolean isVertical, boolean reverse,
			ArrayList<MarbleView> safeZone) {
		int radius = 8;
